...
class ServiceFrame(QFrame):
    ...
    def initialize(self, service):
        ...
        self.acknowledge_btn.setToolTip(_('Acknowledge this service'))
        ...
        self.downtime_btn.setToolTip(_('Schedule a downtime for this service'))
    ...
    def add_services_details(service, layout):
        ...
        if not detailledesc and not impact and not fixactions:
            layout.addWidget(
                QLabel(_('<i>No customs actions defined for this service</i>')),
                row, 2, 1, 4
            )
    ...
    def get_service_icon(self, state):
        ...
        icon_label.setToolTip(_('Service is %s') % state)
    ...
    def get_stars_widget(stars_nb):
        ...
        importance_label = QLabel(_('<b>Importance:</b>'))
        layout.addWidget(importance_label)
        layout.setAlignment(importance_label, Qt.AlignLeft)

        for s in range(  # pylint: disable=unused-variable
                int(get_app_config('Alignak', 'bi_less')), stars_nb):
            star_label = QLabel()
